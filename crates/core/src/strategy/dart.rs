...
impl ReleaseStrategy for DartStrategy {
    fn build_updates(
        ...
        let pubspec_path = join_pkg_path(pkg_path, "pubspec.yaml");
        let pubspec_full = repo_path.join(&pubspec_path);
        if pubspec_full.exists() {
            let content = std::fs::read_to_string(&pubspec_full)?;
            let updated = PUBSPEC_VERSION_RE
...
mod tests {
    ...
    fn test_dart_pubspec_yaml() {
        ...
        assert!(pubspec.content.contains("name: my_pkg"));
    }
}

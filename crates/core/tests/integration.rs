...
fn test_simple_strategy() {
    ...
    let version_update = r
        .file_updates
        .iter()
        .find(|u| u.path == "version.txt")
        .unwrap();
...
fn test_monorepo_mixed_release_types() {
    ...
    let rust = output
        .releases
        .iter()
        .find(|r| r.component.as_deref() == Some("rust-lib"))
        .unwrap();
    assert_eq!(rust.new_version, Version::new(1, 1, 0));
    assert!(rust
        .file_updates
        .iter()
        .any(|u| u.path == "packages/rust-lib/Cargo.toml"));

    let node = output
        .releases
        .iter()
        .find(|r| r.component.as_deref() == Some("node-app"))
        .unwrap();
    assert_eq!(node.new_version, Version::new(1, 0, 1));
    assert!(node
        .file_updates
        .iter()
        .any(|u| u.path == "packages/node-app/package.json"));
}
